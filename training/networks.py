...
class SongUNetPG(SongUNet):
    def __init__(self,
        ...
        self.emb_grad_layer1 = Conv2d(in_channels, model_channels, kernel=1, init_weight=0)  # residual gradient are of the same size as the input
        self.emb_grad_layer2 = Conv2d(model_channels, in_channels, kernel=3, init_weight=0)
    def forward(self, x, noise_labels, class_labels, augment_labels=None, dx=None):
        dx = torch.zeros_like(x) if dx is None else dx
        dx = dx.to(x.dtype)
        dx = self.emb_grad_layer1(dx)
        dx = gelu(dx)
        dx = self.emb_grad_layer2(dx)
        
        x = torch.cat([x, dx], dim=1)
        return super().forward(x, noise_labels, class_labels, augment_labels)

@persistence.persistent_class
class DhariwalUNetPG(DhariwalUNet):
    def __init__(self,
        img_resolution,                     # Image resolution at input/output.
        in_channels,                        # Number of color channels at input.
        out_channels,                       # Number of color channels at output.
