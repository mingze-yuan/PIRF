# https://github.com/jhbastek/PhysicsInformedDiffusionModels/blob/main/src/grad_utils.py
...
def generalized_b_xy_c_to_image(tensor, pixels_x=None, pixels_y=None):
    ...
    pattern = 'b (x y) ' + ' '.join([f'c{i}' for i in range(num_dims)]) + ' -> b ' + ' '.join([f'c{i}' for i in range(num_dims)]) + ' x y'
