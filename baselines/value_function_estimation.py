...
def generate_with_diffusionpde(net, guided_residual_fn, zeta_pde=1, batch_size=16, device='cuda', sigma_min=0.002, sigma_max=80, rho=7, num_steps=80, seed=0):
...
if __name__ == '__main__':
    ...
    output_base_dir = '/n/netscratch/nali_lab_seas/Everyone/mingze/physics_workspace/results/baselines_updated_0510/diffusionpde'
    ...
    zeta_pde_lst = [0, 1e-5, 1e-4, 1e-3, 5e-6]
    num_steps_lst = [40]
    ...
    for zeta_pde in zeta_pde_lst:
        for num_steps in num_steps_lst:
            save_dir = os.path.join(output_dir, f"{model_name}-iter{num_steps}-pde{zeta_pde}")
            os.makedirs(save_dir, exist_ok=True)
            
            lst_res_mse, lst_res_mae = [], []

            for seed in tqdm(seeds):
                x = generate_with_diffusionpde(
                    net=net, 
                ...
                # print(res)
                metrics_file = os.path.join(output_dir, f'{model_name}-iter{num_steps}-pde{zeta_pde}-metrics.json')
