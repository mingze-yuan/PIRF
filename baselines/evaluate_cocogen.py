...
dict_base_model_pth = {
    'darcy': '/n/netscratch/nali_lab_seas/Everyone/mingze/physics_workspace/pretrained_models/diffusionpde/pretrained-models/pretrained-darcy.pkl',
    'darcy64': '/n/netscratch/nali_lab_seas/Everyone/mingze/physics_workspace/workspace/darcy64_pretrain/00001-darcy64-uncond-ddpmpp-edm-gpus2-batch512-fp16/network-snapshot-002560.pkl',
    'burgers': '/n/netscratch/nali_lab_seas/Everyone/mingze/physics_workspace/pretrained_models/diffusionpde/pretrained-models/pretrained-burgers.pkl',
    'poisson': '/n/netscratch/nali_lab_seas/Everyone/mingze/physics_workspace/pretrained_models/diffusionpde/pretrained-models/pretrained-poisson.pkl',
    'helmholtz': '/n/netscratch/nali_lab_seas/Everyone/mingze/physics_workspace/pretrained_models/diffusionpde/pretrained-models/pretrained-helmholtz.pkl',
    'kolmogorov': '/n/netscratch/nali_lab_seas/Everyone/mingze/physics_workspace/workspace/kolmogorov_pretrain/00000-kolmogorov-uncond-ddpmpp-edm-gpus2-batch40-fp16/network-snapshot-006000.pkl',
}

def generate_with_cocogen(net, guided_residual_fn, zeta_pde=1, batch_size=16, device='cuda', sigma_min=0.002, sigma_max=80, rho=7, num_steps=80, seed=0, num_post_correction_steps=1):
    ...
    for i, (sigma_t_cur, sigma_t_next) in tqdm(list(enumerate(zip(sigma_t_steps[:-1], sigma_t_steps[1:]))), unit='step'): # 0, ..., N-1
        ...
        if i > 0.8 * num_steps:
            residual = guided_residual_fn(x_N)
            # in diffusion pde, it uses L2 norm of residual as pde loss
            pde_loss = torch.norm(residual, 2)
            grad_x_cur_pde = torch.autograd.grad(pde_loss, x_cur)[0]
            # print(i, torch.norm(grad_x_cur_pde, 2))
            x_next = x_next - zeta_pde * grad_x_cur_pde
            
    for _ in range(num_post_correction_steps):
        residual = guided_residual_fn(x_next)
        pde_loss = torch.norm(residual, 2)
        grad_x_cur_pde = torch.autograd.grad(pde_loss, x_next)[0]
        x_next = x_next - zeta_pde * grad_x_cur_pde
...
if __name__ == '__main__':
    ...
    output_base_dir = '/n/netscratch/nali_lab_seas/Everyone/mingze/physics_workspace/results/baselines_updated_0511/cocogen'
    ...
    total_samples = 1600
    # num_steps = 20
    num_post_correction_steps_lst = [1, 3, 5]
    # batch_size = 8
    num_steps_lst = [40]
    
    zeta_pde_lst = [1e-5, 1e-4, 1e-3, 5e-6]
    ...
    net.eval()
    for zeta_pde in zeta_pde_lst:
        for num_steps in num_steps_lst:
            for num_post_correction_steps in num_post_correction_steps_lst:
                save_dir = os.path.join(output_dir, f"{model_name}-iter{num_steps}-pde{zeta_pde}-post{num_post_correction_steps}")
                os.makedirs(save_dir, exist_ok=True)
                
                lst_res_mse, lst_res_mae = [], []

                for seed in tqdm(seeds):
                    x = generate_with_cocogen(
                        net=net, 
                        ...
                        seed=seed, 
                        num_steps=num_steps,
                        num_post_correction_steps=num_post_correction_steps)
                    # x = postprocess_darcy(x.detach())
                    # x = postprocess_burgers(x.detach())
                    ...
                    # print(res)
                    metrics_file = os.path.join(output_dir, f'{model_name}-iter{num_steps}-pde{zeta_pde}-post{num_post_correction_steps}-metrics.json')
