...
dict_base_model_pth = {
    'darcy': '/n/netscratch/nali_lab_seas/Everyone/mingze/physics_workspace/workspace_pidm/darcy/00002-darcy-uncond-ddpmpp-edm-gpus2-batch128-fp16/network-snapshot-001228.pkl',
    'burgers': '/n/netscratch/nali_lab_seas/Everyone/mingze/physics_workspace/workspace_pidm/burgers/00002-burgers-uncond-ddpmpp-edm-gpus2-batch128-fp16/network-snapshot-001228.pkl',
    'helmholtz': '/n/netscratch/nali_lab_seas/Everyone/mingze/physics_workspace/workspace_pidm/helmholtz/00001-helmholtz-uncond-ddpmpp-edm-gpus2-batch128-fp16/network-snapshot-001249.pkl',
    'poisson': '/n/netscratch/nali_lab_seas/Everyone/mingze/physics_workspace/workspace_pidm/poisson/00001-poisson-uncond-ddpmpp-edm-gpus2-batch128-fp16/network-snapshot-001269.pkl',
    'kolmogorov': '/n/netscratch/nali_lab_seas/Everyone/mingze/physics_workspace/workspace_pidm/kolmogorov/00004-kolmogorov-uncond-ddpmpp-edm-gpus2-batch40-fp16/network-snapshot-006220.pkl',
}

def generate_with_pidm(net, batch_size=16, device='cuda', sigma_min=0.002, sigma_max=80, rho=7, num_steps=80, seed=0):
...
if __name__ == '__main__':
    ...
    output_base_dir = '/n/netscratch/nali_lab_seas/Everyone/mingze/physics_workspace/results/baselines_updated/pidm'
    output_dir = os.path.join(output_base_dir, args.data_name)
    os.makedirs(output_dir, exist_ok=True)
    
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    eval_residual_fn, _, batch_size = get_required_tools(args.data_name, device)
    total_samples = 1600
    ...
    for num_steps in num_steps_lst:
        ...
        for seed in tqdm(seeds):
            ...
            # print(res)
            metrics_file = os.path.join(output_dir, f'{model_name}-iter{num_steps}-metrics.json')
